...
def plot_box_plot(data, title='Box Plot', ylabel='Value', horizontal=False, labels=None):
    ...
    if horizontal:
        fig.update_xaxes(tickformat=",~f", tickmode='linear', tickfont=dict(size=14), showgrid=True, gridwidth=1, gridcolor='rgba(200,200,200,0.3)')
    else:
        fig.update_yaxes(tickformat=",~f", tickmode='linear', tickfont=dict(size=14), showgrid=True, gridwidth=1, gridcolor='rgba(200,200,200,0.3)')
    return fig
